...
final messagesRepoProvider = Provider<MessagesRepository>((_) => MessagesRepository());
final apiProvider = Provider<ApiClient>((_) => ApiClient.auto());

final messagesProvider = StreamProvider.family<List<MessageModel>, String>((ref, subject) {
...
class ChatRepo extends ChangeNotifier {
  ...
  Future<String> send(String subject, String question) async {
    ...
    await repo.insertMessage(MessageModel(
        uid: uid, subject: subject, role: 'user', content: question, createdAt: DateTime.now()));

    final res = await http.post('/v1/chat', data: {
      'subject': subject,
      'message': question,
    ...
    await repo.insertMessage(MessageModel(
        uid: uid, subject: subject, role: 'model', content: reply, createdAt: DateTime.now()));
